(function () {
  ...
  const exercise = { id: "pembagian_lanjutan_01", name: "Pembagian Lanjutan", durationSeconds: 60 };
  ...
  if (!student) {
    window.location.replace("../student.html?next=latihan/pembagian-lanjutan.html");
    return;
  }
  ...
  function createQuestion() {
    ...
    do {
      ...
      const minimumQuotient = Math.max(10, Math.ceil(100 / divisor));
      quotient = Math.floor(Math.random() * (100 - minimumQuotient)) + minimumQuotient;
  ...
  answerForm.addEventListener("submit", function (event) {
    ...
    if (!/^\d{1,2}$/.test(answerText)) {
      answerMessage.textContent = "Masukkan jawaban berupa angka dua digit.";
  ...
  window.MathPractice.startAdvancedDivisionExercise = startExercise;
  window.MathPractice.retryAdvancedDivisionResult = saveCurrentResult;
})();
